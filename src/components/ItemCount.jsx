    ...
    <>
      {eventButton ?
        <div>
          ...
          <div className={'d-flex flex-row justify-content-center'}>
            <button className={"btn btn-info"} onClick={() => { onAdd(count); changeButton() }}>Agregar</button>
          </div>
        </div> : 
        <div className={'d-flex flex-row justify-content-center'}><Link to='/Cart'> <button className={"btn btn-info"}>Ir a la cesta</button> </Link></div>
      }
    </>
  )
}
